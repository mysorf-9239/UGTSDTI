...
def build_drug_drug_graph(
    ...
    if graph.num_nodes != len(smiles_list):
        raise ValueError(
            f"num_nodes mismatch: graph.num_nodes={graph.num_nodes}, "
            f"len(smiles_list)={len(smiles_list)}"
        )
    return graph
...
def build_protein_protein_graph(
    ...
    if graph.num_nodes != len(fasta_list):
        raise ValueError(
            f"num_nodes mismatch: graph.num_nodes={graph.num_nodes}, "
            f"len(fasta_list)={len(fasta_list)}"
        )
    return graph
