"""Extended loss tests: BCELoss, KDLoss edge cases."""
...
def test_bce_wrapper_ignores_extra_keys():
    """BCELoss must only use 'logits' key, ignore student/teacher logits."""
    loss_fn = BCELoss()
...
def test_kd_loss_fallback_no_student_teacher():
    """Without student/teacher logits, KDLoss falls back to pure BCE."""
...
def test_kd_loss_alpha_range(alpha):
    """KDLoss must be finite for all valid alpha values."""
    loss_fn = KDLoss(alpha=alpha)
    out = {"logits": torch.randn(4), "student_logits": torch.randn(4), "teacher_logits": torch.randn(4)}
...
def test_kd_loss_gradient_flows_to_student():
    ...
    t_logits = torch.randn(2)
    labels = torch.randint(0, 2, (2,)).float()

    loss = loss_fn({"logits": s_logits, "student_logits": s_logits, "teacher_logits": t_logits}, labels)
    loss.backward()
