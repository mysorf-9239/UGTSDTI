...
def test_tdc_caching_and_batching(MockAutoTokenizer):
    ...
    assert "drug_index" in sample, "Loader must output sequential drug node index"
    assert "target_index" in sample, "Loader must output sequential target node index"
