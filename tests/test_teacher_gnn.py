...
@given(
    smiles_list=st.lists(
        st.sampled_from(VALID_SMILES_POOL), min_size=1, max_size=10
    )
)
...
@given(
    smiles_list=st.lists(
        st.sampled_from(VALID_SMILES_POOL), min_size=2, max_size=10
    )
)
@settings(max_examples=50)
...
@given(
    smiles_list=st.lists(
        st.sampled_from(VALID_SMILES_POOL), min_size=1, max_size=5
    ),
    fasta_list=st.lists(
...
@settings(max_examples=50)
def test_p5_graph_cache_roundtrip(smiles_list, fasta_list, tmp_path):
...
@given(
    old_edges=st.frozensets(
        st.tuples(st.integers(0, 9), st.integers(0, 9))
    ),
    new_edges=st.frozensets(
        st.tuples(st.integers(0, 9), st.integers(0, 9))
    ),
)
@settings(max_examples=50)
